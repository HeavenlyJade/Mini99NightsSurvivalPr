...
local ClientScheduler = require(MainStorage.code.client.clientEvent.ClientScheduler) ---@type ClientScheduler
    ...
    for i, part in ipairs(parts) do
        if i == #parts then
            ...
            if mark then
                ...
                gg.log('part',part,type(part),current)
                current[part] = nil
            end
        else
            -- 中间部分
            if mark then
                ...
                if not current[part] or type(current[part]) == "boolean" then

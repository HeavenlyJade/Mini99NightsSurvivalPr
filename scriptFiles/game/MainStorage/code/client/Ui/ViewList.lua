...
---@return number
function ViewList:GetChilds()
    return self.childrens
end

function ViewList:cleanChildsSelect(RolePaths)
    for _, child_ in ipairs(self.childrens) do
        if child_.node then
            child_.node[RolePaths.SelectImg].Visible = false
            child_.node[RolePaths.YellowSelect].Visible = false
        end
    end
end
